...
for scripts_dir in "${SCRIPTS_DIRS[@]}"; do
  pushd "${PWD}" >/dev/null 2>&1 || die $? "Failed during 'pushd ${PWD}'"
  cd "${scripts_dir}" >/dev/null 2>&1 || die $? "Failed during 'cd ${scripts_dir}'"
  ...
  for test_dir in "${TESTS_DIRS[@]}"; do
    ...
    LIVE_FOREVER=1
    run_cmd 0 "${TEST_TOOL}" "${TEST_PARAMS[@]}" "${test_dir}" && rc=$? || rc=$?
    LIVE_FOREVER=0
    if [[ "${rc}" -eq 0 ]]; then
  ...
  popd >/dev/null 2>&1 || die 1 "Failed during 'popd' with exit code ${?}"
done
...
for scripts_dir in "${SCRIPTS_DIRS[@]}"; do
  ...
  while read -r filename; do
    ...
    LIVE_FOREVER=1
    run_cmd 0 "${LINT_TOOL}" "${LINT_PARAMS[@]}" "${filename}" > "${report_file}" && rc=$? || rc=$?
    LIVE_FOREVER=0
    if [[ "${rc}" -eq 0 ]]; then
