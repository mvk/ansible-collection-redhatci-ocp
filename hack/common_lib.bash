  ...
  case "${os}" in
    Linux)
      if ! command -v grep &> /dev/null; then
        rc=$?
        die "${rc}" "grep is not installed, install it with 'sudo [dnf|yum|apt] -y install grep'"
      fi
      GREP="grep"
      if ! command -v find &> /dev/null; then
        rc=$?
        die "${rc}" "find is not installed, install it with 'sudo [dnf|yum|apt] -y install findutils'"
      fi
      FIND="find"
      if ! command -v sed &> /dev/null; then
        rc=$?
        die "${rc}" "sed is not installed, install it with 'sudo [dnf|yum|apt] -y install sed'"
      fi
      SED="sed"
    ;;
    Darwin)
      if ! command -v ggrep &> /dev/null; then
        rc=$?
        die "${rc}" "ggrep is not installed, install it with 'brew install grep'"
      fi
      GREP="ggrep"
      if ! command -v gfind &> /dev/null; then
        rc=$?
        die "${rc}" "gfind is not installed, install it with 'brew install findutils'"
      fi
      FIND="gfind"
      if ! command -v gsed &> /dev/null; then
        rc=$?
        die "${rc}" "gsed is not installed, install it with 'brew install gnu-sed'"
      fi
      SED="gsed"
    ;;
