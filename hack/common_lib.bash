...
# Returns:
#   0 on success, calls log.die() on invalid level
#######################################
function log.set_level() {
  ...
  if ! contains_element "${level}" "${!LEVELS_MAP[@]}"; then
    log.die 1 "Invalid log level: ${level}"
  fi
  LOG_LEVEL="${level}"
  return 0
}
...
# Returns:
#   Expected return code on success, calls log.die() on failure
#######################################
function utils.run_cmd() {
  ...
  shift 1
  cmd=("${@}")
  test "${#cmd[@]}" -ne 0 || log.die 1 "The command '${cmd[*]}' cannot be empty"
...
#   Debug and error messages to stderr
# Returns:
#   0 on success, calls log.die() on unsupported OS
#######################################
function utils.tools_setup() {
  ...
  case "${os}" in
    Linux)
      if ! command -v grep &> /dev/null; then
        rc=$?
        log.die "${rc}" "grep is not installed, install it with 'sudo [dnf|yum|apt] -y install grep'"
      fi
      GREP="grep"
      if ! command -v find &> /dev/null; then
        rc=$?
        log.die "${rc}" "find is not installed, install it with 'sudo [dnf|yum|apt] -y install findutils'"
      fi
      FIND="find"
      if ! command -v sed &> /dev/null; then
        rc=$?
        log.die "${rc}" "sed is not installed, install it with 'sudo [dnf|yum|apt] -y install sed'"
      fi
      SED="sed"
    ;;
    Darwin)
      if ! command -v ggrep &> /dev/null; then
        rc=$?
        log.die "${rc}" "ggrep is not installed, install it with 'brew install grep'"
      fi
      GREP="ggrep"
      if ! command -v gfind &> /dev/null; then
        rc=$?
        log.die "${rc}" "gfind is not installed, install it with 'brew install findutils'"
      fi
      FIND="gfind"
      if ! command -v gsed &> /dev/null; then
        rc=$?
        log.die "${rc}" "gsed is not installed, install it with 'brew install gnu-sed'"
      fi
      SED="gsed"
    ;;
  ...
  rc=0
  epilog "${rc}" "Running on ${os}. Selected: GREP=${GREP}, FIND=${FIND}, SED=${SED}"
  return 0
}
...
function utils.hr() {
  ...
  width="${1:-"80"}"
  char="${2:-"~"}"
  echo -e "$(printf "%${width}s" | tr ' ' "${char}" || true)"
  return 0
}

function utils.decorated_message() {
  local \
    message \
    width \
    char
  message="${1?cannot continue without message}"
  width="${2:-""}"
  char="${3:-""}"
  utils.hr "${width}" "${char}"
  echo -e "${message}"
...
#######################################
# Ensure that the given apps are installed
# Arguments:
#   $1+ - Apps to check
# Outputs:
#   Debug and error messages to stderr
#######################################
function utils.ensure_apps() {
  local \
    app \
    rc
  local -a apps=("${@}")
  for app in "${apps[@]}"; do
    if ! command -v "${app}" &> /dev/null; then
      rc=$?
      log.die "${rc}" "${app} is not installed, install it with 'sudo [dnf|yum|apt] -y install ${app}'"
    fi
  done
  return 0
}
...
function utils.print_apps_versions() {
  local \
    app \
    version
  local -a apps=("${@}")
  utils.decorated_message "Checking for versions of applications: $(join_by ", " "${apps[@]}" || true)"
  for app in "${apps[@]}"; do
    version="$("${app}" --version || "${app}" version || true)"
    echo -e "${app}: ${version}"
  done
  utils.decorated_message "Done checking for versions of applications"
  return 0
}

# print the installed python modules with the given prefixes
function utils.print_python_modules() {
  local \
    prefix \
    message
  local -a \
  ...
  if [[ "${#prefixes[@]}" -eq 0 ]]; then
    log.warning "No prefixes provided"
  fi
  message="Show installed python modules"
  local -a cmds_pipeline
  cmds_pipeline=("pip freeze")
  prefix="$(join_by "|" "${prefixes[@]}" || true)"
  if [[ -n "${prefix}" ]]; then
    message+=" (named with prefix: ${prefix})"
  fi
  utils.decorated_message "${message}"
  if [[ -n "${prefix}" ]]; then
    cmds_pipeline+=("${GREP} -E \"^(${prefix})\"")
  fi
  local cmd
  cmd="$(join_by " | " "${cmds_pipeline[@]}" || true)"
  log.debug "About to run: '${cmd}'"
  eval "${cmd}" || true
  utils.decorated_message "Done showing installed python modules"
  return 0
}
...
#######################################
function utils.compare_versions() {
  local \
    left \
    right \
    left_major \
...
# switch to a branch optionally updating it from the remote
function git.switch_to_branch() {
  local \
    branch \
    remote \
    current_branch \
    rc
  branch="${1?cannot continue without branch}"
  remote="${2:-"origin"}"
  current_branch="$(git rev-parse --abbrev-ref HEAD || true)"
  rc=0
  if [[ "${current_branch}" != "${branch}" ]]; then
    log.info "Switching to ${branch} branch"
    if [[ "${GIT_SYNC}" -gt 0 ]]; then
      utils.run_cmd 0 git fetch "${remote}" "${branch}"
      rc=$?
      log.debug "Updated branch ${branch}"
      log.trace "Updated branch: ${branch} from remote: ${remote} with rc: ${rc}"
    fi
    utils.run_cmd 0 git checkout "${branch}"
    rc=$?
    log.debug "Switched to ${branch}"
    log.trace "Switched to branch: ${branch} from remote: ${remote} with rc: ${rc}"
  fi
  return "${rc}"
}

# apply a list of patches onto the current branch
function git.apply_patches() {
  local \
    patch_file \
    applied_patches \
    total_patches \
  ...
  local -n \
    patches_list_ref
  patches_list_ref="${1:-"PRE_TEST_PATCHES"}"
  applied_patches=0
  total_patches="${#patches_list_ref[@]}"
  rc=0
  if [[ "${total_patches}" -eq 0 ]]; then
    log.warning "No patches to apply"
    return "${rc}"
  fi
  log.debug "Applying ${total_patches} patches: '$(join_by "', '" "${patches_list_ref[@]}" || true)'"
  # apply patches
  for patch_file in "${patches_list_ref[@]}"; do
    if ! [[ -r "${patch_file}" ]]; then
      log.warning "Patch file: ${patch_file} is missing"
      continue
    fi
    git apply "${patch_file}"
    rc=$?
    if [[ "${rc}" -ne 0 ]]; then
      log.error "Failed to apply patch [$((applied_patches +1)) / ${total_patches}] named: ${patch_file} with rc: ${rc}"
      return "${rc}"
    fi
    applied_patches=$((applied_patches + 1))
  done
  log.info "Applied [${applied_patches} / ${total_patches}] patches with rc: ${rc}"
  return "${rc}"
...
if [[ "${BASH_SOURCE[0]}" = "${0}" ]]; then
  log.die 1 "This script is not meant to be run directly"
fi
