...
GIT_SYNC="${GIT_SYNC:-"1"}"
# RUN_IN_PARALLEL: run the tests in parallel (default: 0 - no)
...
# TEST_PIDS: array of PIDs of the tests running in parallel
declare -a TEST_PIDS

# --- Library loading ---
TOPDIR="$(git rev-parse --show-toplevel || true)"
test -z "${TOPDIR}" && {
  echo "FATAL: This script expects to run from a git repository" >&2
  exit 1
}
...
function run_tests() {
  ...
  for test_type in "${test_types_ref[@]}"; do
    ...
    run_cmd 0 truncate -s 0 "${report_file}"
    message="[RUN_IN_PARALLEL=${RUN_IN_PARALLEL}] Running ${test_type} tests for ${version} and saving the output to ${report_file}"
...
function run_tests_on_branch() {
  ...
  switch_to_branch "${branch}" "${remote}"
  # apply patches
  apply_patches PRE_TEST_PATCHES
...
function generate_report() {
  ...
  options=("${@}")
  rc=0
  run_cmd 0 truncate -s 0 "${target_report}"
  ...
  for option in "${options[@]}"; do
    ...
    run_cmd 0 truncate -s 0 "${curr_errors_file}"
