...
def _collect_failure(testcase_elem: etree.Element) -> Optional[Dict[str, Any]]:
    ...
    # Use the type attribute if present, otherwise default to "failure"
    failure_type = failure_elem.get("type")
    result_dict["status"] = failure_type if failure_type else "failure"
...
def _collect_error(testcase_elem: etree.Element) -> Optional[Dict[str, Any]]:
    ...
    if error_elem.text:
        ...
        result_dict["status"] = "panicked" if "[PANICKED]" in text else "error"
    else:
